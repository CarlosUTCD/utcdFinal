function guardarProducto(){
  ...
  // 1) Restringir la tecla: sólo dígitos y un punto decimal
  $("#precio").on("keypress", function(e){
    const char = String.fromCharCode(e.which);
    // Si no es dígito ni punto, bloquea
    if (!/[0-9.]/.test(char)) {
      e.preventDefault();
    }
    // Si ya había un punto, no dejar otro
...
function validarPrecio() {
  ...
  // patrón: uno o más dígitos, opcional “.” seguido de dígitos
  if (!/^\d+(\.\d+)?$/.test(precio)) {
